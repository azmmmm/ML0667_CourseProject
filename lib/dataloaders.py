...
from lib.utils import ImageTransforms,ImageTransforms_v2
...
class SRDataset_v2(Dataset):
    """
    数据集加载器v2
    在下采样的同时，增加了高斯模糊和JPEG压缩处理
    """

    def __init__(self, data_folder, split, crop_size, scaling_factor, lr_img_type, hr_img_type, test_data_name=None,gasuss_noise_sigma=0.001):
        """
        :参数 data_folder: # Json数据文件所在文件夹路径
        :参数 split: 'train' 或者 'test'
        :参数 crop_size: 高分辨率图像裁剪尺寸  （实际训练时不会用原图进行放大，而是截取原图的一个子块进行放大）
        ...
        # 数据处理方式
        self.transform = ImageTransforms_v2(split=self.split,
                                         crop_size=self.crop_size,
                                         scaling_factor=self.scaling_factor,
                                         lr_img_type=self.lr_img_type,
                                         hr_img_type=self.hr_img_type,
                                         gasuss_noise_sigma=self.gasuss_noise_sigma)
